...
Vagrant.configure(2) do |config|
  ...
  config.vm.hostname = 'chef-server.example.com'
  config.vm.network 'private_network', ip: '192.168.33.199'
  config.vm.provision "shell", inline: "apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y unzip curl wget"
  ...
  config.vm.provision "shell", inline: "echo 192.168.33.199 chef-server.example.com | sudo tee -a /etc/hosts"
  ...
  config.vm.provision "shell", inline: "echo 'Server is up. Please log in at https://chef-server.example.com/'"
